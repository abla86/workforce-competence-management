...
internal sealed class RoleGuardStartupFilter : IStartupFilter
{
    ...
    private static async Task ImportCompetencesAsync(HttpContext context, AppDbContext db)
    {
        ...
        for (var r = 1; r < rows.Count; r++)
        {
            ...
            var employee = await db.Employees.FirstOrDefaultAsync(e => e.Name == employeeName); var competence = await db.Competences.FirstOrDefaultAsync(c => c.Name == competenceName);
            if (employee is null || competence is null) { errors.Add(new { row = r + 1, message = "EmployeeName or CompetenceName not found." }); continue; }
            var levelText = Get("Level"); if (string.IsNullOrWhiteSpace(levelText)) levelText = nameof(CompetenceLevel.Basic);
            if (!Enum.TryParse<CompetenceLevel>(levelText, true, out var level)) { errors.Add(new { row = r + 1, message = $"Invalid competence level '{levelText}'. Valid values: Basic, Intermediate, Advanced." }); continue; }
