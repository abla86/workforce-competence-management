...
public sealed class EmployeeAvailabilityService : IEmployeeAvailabilityService
{
    ...
    public async Task<EmployeeStatus> GetEmployeeStatusAsync(int employeeId, DateTime date)
    {
        ...
        if (absence is not null)
            return new EmployeeStatus { EmployeeId = employeeId, EmployeeName = employee.Name, Status = absence.Type switch { AbsenceType.SickLeave => EmployeeAvailabilityStatus.Sick, AbsenceType.Vacation => EmployeeAvailabilityStatus.OnVacation, _ => EmployeeAvailabilityStatus.Away }, StatusText = GetAbsenceLabel(absence), StartTime = dayStart, EndTime = dayEnd, IsAutomatic = true };
        ...
        return manual ?? new EmployeeStatus { EmployeeId = employeeId, EmployeeName = employee.Name, Status = EmployeeAvailabilityStatus.Available, StatusText = "Fri", StartTime = dayStart, EndTime = dayEnd, IsAutomatic = true };
    }
    ...
    public async Task SetEmployeeStatusAsync(int employeeId, EmployeeAvailabilityStatus status, string? statusText, DateTime? endTime, string changedByUserId)
    {
        ...
        var item = await _db.EmployeeStatuses.FindAsync(employeeId) ?? new EmployeeStatus { EmployeeId = employeeId };
        if (item.EmployeeId == 0) _db.EmployeeStatuses.Add(item);
...
public sealed class DailyPlanService : IDailyPlanService
{
    ...
    {
        var existing = await GetTodayPlanAsync(departmentId); if (existing is not null) return existing;
        ...
        _db.DailyPlans.Add(plan); await _db.SaveChangesAsync(); return plan;
    }
    public async Task<DailyPlan> PublishDailyPlanAsync(int planId, string userId)
    {
        ...
        await _db.SaveChangesAsync(); await _notifications.NotifyDepartmentAsync(plan.DepartmentId, "Ny dagsplan", $"Ny dagsplan er publisert for {plan.PlanDate:dd.MM.yyyy}", NotificationType.DailyPlanPublished); return plan;
    }
...
public sealed class ShiftPlanService : IShiftPlanService
{
    ...
    public async Task<ShiftPlan> PublishShiftPlanAsync(int planId, string userId)
    {
        ...
        await _db.SaveChangesAsync(); await _notifications.NotifyDepartmentAsync(plan.DepartmentId, "Ny vaktplan publisert", "En ny vaktplan er publisert. Sjekk vakten din.", NotificationType.ShiftPlanPublished); return plan;
    }
    ...
    public async Task<List<Employee>> GetWhoIsWorkingTodayAsync(int departmentId, DateTime date)
    {
        var assignmentIds = await _db.ShiftAssignments.Where(a => a.Shift.StartTime.Date == date.Date).Select(a => a.EmployeeId).ToListAsync();
        ...
        var ids = assignmentIds.Concat(coverageIds).Distinct().ToList();
        return await _db.Employees.Where(e => e.DepartmentId == departmentId && e.IsActive && ids.Contains(e.Id)).Include(e => e.ShiftAssignments).ThenInclude(a => a.Shift).ToListAsync();
    }
}
