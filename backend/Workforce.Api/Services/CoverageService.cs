...
public sealed class CoverageService
{
    public ShiftCoverageResult AnalyzeShift(Shift shift)
    {
        ...
        var requirementResults = shift.Requirements.Select(requirement =>
        {
            ...
            return new RequirementCoverageResult(
                ...
                requirement.MinimumLevel.ToString(),
                qualified,
                covered,
        ...
        return new ShiftCoverageResult(
            ...
            shift.Assignments.OrderBy(x => x.Employee.Name)
                .Select(x => new ShiftAssignmentResult(x.EmployeeId, x.Employee.Name, x.Employee.Role))
                .ToList(),
            requirementResults,
            warnings);
    }
    ...
    private async Task AddAvailabilityWarningsAsync(AppDbContext db, Shift shift, List<string> warnings)
    {
        ...
        foreach (var assignment in shift.Assignments)
        {
            ...
            var employeeShifts = otherShifts
                .Where(s => s.Assignments.Any(a => a.EmployeeId == employee.Id))
                .ToList();

            if (employeeShifts.Any(other =>
                    SchedulingRules.GetStart(other) < end &&
                    SchedulingRules.GetEnd(other) > start))
    ...
    private static bool IsQualified(Employee employee, ShiftRequirement requirement, DateOnly shiftDate)
    {
        ...
        return ec is not null &&
               ec.Level >= requirement.MinimumLevel &&
