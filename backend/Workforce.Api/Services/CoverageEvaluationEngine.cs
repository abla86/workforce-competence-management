...
public sealed class CoverageEvaluationEngine
{
    ...
    public async Task<CoverageResult> EvaluateAsync(int shiftId, string triggeredBy = "system", bool writeAudit = true)
    {
        ...
        if (writeAudit) await LogAuditAsync(shiftId, result, triggeredBy);
        return result;
    }
    ...
    public async Task<CoverageResult> EvaluateScenarioWithoutEmployeesAsync(int shiftId, IReadOnlyCollection<int> employeeIds)
    {
        ...
        var result = new CoverageResult();
        foreach (var task in shift.ShiftTasks) result.Tasks.Add(await EvaluateTaskAsync(shift, task));
    ...
    public async Task<List<SuggestedReplacement>> FindQualifiedReplacementsAsync(int shiftId, IReadOnlyCollection<int> excludedEmployeeIds)
    {
        ...
        var candidates = await _db.Employees.Include(e => e.Competences).Include(e => e.Availability)
            .Where(e => e.IsActive && !excluded.Contains(e.Id) && !assigned.Contains(e.Id)).ToListAsync();
        ...
        foreach (var employee in candidates)
        {
            ...
            foreach (var task in shift.ShiftTasks)
            {
                ...
                if (task.WorkTask.CompetenceId is int competenceId)
                {
                    ...
                    if (competence is null) { missing.Add($"Mangler kompetanse: {task.WorkTask.Name}"); continue; }
                    var level = LevelRank(competence.Level); levels.Add(level);
            ...
            replacements.Add(new SuggestedReplacement { EmployeeId = employee.Id, EmployeeName = employee.Name, Role = employee.Role, CompetenceLevel = levels.DefaultIfEmpty(0).Max(), Available = missing.Count == 0, MissingRequirements = missing.Distinct().ToList() });
        }
        return replacements.OrderByDescending(x => x.Available).ThenBy(x => x.MissingRequirements.Count).ThenBy(x => x.EmployeeName).ToList();
    }
    ...
    private async Task<TaskCoverageDetail> EvaluateTaskAsync(Shift shift, ShiftTask task)
    {
        ...
        foreach (var coverage in task.ShiftTaskCoverages)
        {
            ...
            {
                if (await HasSchedulingConflictAsync(coverage.EmployeeId, shift)) gaps.Add(new CoverageGap { Type = GapType.DoubleBooked, EmployeeId = coverage.EmployeeId, EmployeeName = coverage.Employee.Name, Description = $"{coverage.Employee.Name} har overlappende vakt" });
                if (await ViolatesRestAsync(coverage.EmployeeId, shift)) gaps.Add(new CoverageGap { Type = GapType.RestPeriodViolation, EmployeeId = coverage.EmployeeId, EmployeeName = coverage.Employee.Name, Description = $"{coverage.Employee.Name} har mindre enn {DefaultMinimumRestHours} timer hvile" });
            }
    ...
    private static CoverageStatus DetermineOverallStatus(CoverageResult result, bool understaffed) => result.Tasks.Any(t => t.Critical && t.Gaps.Any()) ? CoverageStatus.Red : understaffed || result.Tasks.Any(t => t.Gaps.Any()) ? CoverageStatus.Yellow : CoverageStatus.Green;

    private async Task LogAuditAsync(int shiftId, CoverageResult result, string triggeredBy)
    {
        var entry = new CoverageAuditEntry { ShiftId = shiftId, EvaluatedAt = DateTime.UtcNow, Status = result.Status.ToString(), TriggeredBy = triggeredBy };
