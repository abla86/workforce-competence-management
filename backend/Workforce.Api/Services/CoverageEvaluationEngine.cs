...
public sealed class CoverageEvaluationEngine
{
    ...
    {
        var shift = await LoadShiftAsync(shiftId) ?? throw new ArgumentException($"Shift {shiftId} not found");
        ...
        if (understaffed) result.Warnings.Add($"Vakten har {shift.Assignments.Count} ansatte, men krever minst {shift.MinimumStaff}.");
    ...
    public async Task<List<SuggestedReplacement>> FindQualifiedReplacementsAsync(int shiftId, IReadOnlyCollection<int> excludedEmployeeIds)
    {
        ...
        var candidates = await _db.Employees.Include(x => x.Competences).Include(x => x.Availability)
            .Where(x => x.IsActive && !excluded.Contains(x.Id) && !assigned.Contains(x.Id)).ToListAsync();
        ...
        foreach (var employee in candidates)
        {
            ...
            foreach (var task in shift.ShiftTasks)
            {
                if (task.WorkTask.CompetenceId is int competenceId)
                {
                    ...
                    if (competence is null) missing.Add($"Mangler kompetanse: {task.WorkTask.Name}");
                    else
                    {
                ...
                if (!string.IsNullOrWhiteSpace(task.WorkTask.RequiredAuthorization))
                {
                    if (!string.Equals(employee.Authorization, task.WorkTask.RequiredAuthorization, StringComparison.OrdinalIgnoreCase)) missing.Add($"Krever autorisasjon: {task.WorkTask.RequiredAuthorization}");
                    else if (employee.AuthorizationExpiry.HasValue && employee.AuthorizationExpiry.Value < shift.StartTime) missing.Add($"Utløpt autorisasjon: {task.WorkTask.RequiredAuthorization}");
                }
            }
            ...
            result.Add(new SuggestedReplacement
            {
                ...
                AvailableSlots = missing.Count == 0 ? 1 : 0,
    ...
    {
        var detail = new TaskCoverageDetail { TaskName = task.WorkTask.Name, CompetenceName = task.WorkTask.Competence?.Name ?? "", Required = task.RequiredCount, Critical = task.IsCritical };
        foreach (var coverage in task.ShiftTaskCoverages)
        {
            if (!coverage.IsValid)
            {
                detail.Gaps.Add(new CoverageGap { Type = GapType.InvalidCoverage, EmployeeId = coverage.EmployeeId, EmployeeName = coverage.Employee.Name, Description = string.IsNullOrWhiteSpace(coverage.InvalidReason) ? $"Dekningen for {coverage.Employee.Name} er markert som ugyldig." : coverage.InvalidReason });
                continue;
            }
            var gaps = ValidateCoverage(shift, task, coverage);
            if (gaps.Count == 0 && await HasSchedulingConflictAsync(coverage.EmployeeId, shift)) gaps.Add(new CoverageGap { Type = GapType.DoubleBooked, EmployeeId = coverage.EmployeeId, EmployeeName = coverage.Employee.Name, Description = $"{coverage.Employee.Name} har overlappende vakt" });
            if (gaps.Count == 0 && await ViolatesRestAsync(coverage.EmployeeId, shift)) gaps.Add(new CoverageGap { Type = GapType.RestPeriodViolation, EmployeeId = coverage.EmployeeId, EmployeeName = coverage.Employee.Name, Description = $"{coverage.Employee.Name} har mindre enn {MinimumRestHours} timer hvile" });
    ...
    private static List<CoverageGap> ValidateCoverage(Shift shift, ShiftTask task, ShiftTaskCoverage coverage)
    {
        ...
        if (!string.IsNullOrWhiteSpace(task.WorkTask.RequiredRole) && !string.Equals(employee.Role, task.WorkTask.RequiredRole, StringComparison.OrdinalIgnoreCase)) gaps.Add(new CoverageGap { Type = GapType.UnauthorizedRole, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} har rolle {employee.Role}, krever {task.WorkTask.RequiredRole}" });

        if (task.WorkTask.CompetenceId is int competenceId)
        {
            var competence = employee.Competences.FirstOrDefault(x => x.CompetenceId == competenceId);
            if (competence is null) gaps.Add(new CoverageGap { Type = GapType.MissingCompetence, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} mangler kompetanse for {task.WorkTask.Name}" });
            else
            {
                if (LevelRank(competence.Level) < task.MinCompetenceLevel) gaps.Add(new CoverageGap { Type = GapType.MissingCompetence, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} har nivå {competence.Level}, krever minst {task.MinCompetenceLevel}" });
                if (competence.ValidUntil.HasValue && competence.ValidUntil.Value < shift.Date) gaps.Add(new CoverageGap { Type = GapType.CompetenceExpired, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} sin kompetanse utløp {competence.ValidUntil:dd.MM.yyyy}" });
            }
        }

        if (!string.IsNullOrWhiteSpace(task.WorkTask.RequiredAuthorization))
        {
            if (!string.Equals(employee.Authorization, task.WorkTask.RequiredAuthorization, StringComparison.OrdinalIgnoreCase)) gaps.Add(new CoverageGap { Type = GapType.UnauthorizedRole, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} mangler autorisasjon {task.WorkTask.RequiredAuthorization}" });
            else if (employee.AuthorizationExpiry.HasValue && employee.AuthorizationExpiry.Value < shift.StartTime) gaps.Add(new CoverageGap { Type = GapType.AuthorizationExpired, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} sin autorisasjon {task.WorkTask.RequiredAuthorization} er utgått" });
        }
        if (coverage.AuthorizationExpiry.HasValue && coverage.AuthorizationExpiry.Value < shift.StartTime) gaps.Add(new CoverageGap { Type = GapType.AuthorizationExpired, EmployeeId = employee.Id, EmployeeName = employee.Name, Description = $"{employee.Name} sin autorisasjon for oppgaven er utgått" });
        return gaps;
    }

    private async Task<bool> HasSchedulingConflictAsync(int employeeId, Shift shift) => await _db.Shifts.AnyAsync(s => s.Id != shift.Id && s.StartTime < shift.EndTime && s.EndTime > shift.StartTime && (s.Assignments.Any(a => a.EmployeeId == employeeId) || s.ShiftTasks.Any(st => st.ShiftTaskCoverages.Any(sc => sc.EmployeeId == employeeId))));

    private async Task<bool> ViolatesRestAsync(int employeeId, Shift shift)
    {
        var previous = await _db.Shifts.Where(s => s.Id != shift.Id && s.EndTime <= shift.StartTime && (s.Assignments.Any(a => a.EmployeeId == employeeId) || s.ShiftTasks.Any(st => st.ShiftTaskCoverages.Any(sc => sc.EmployeeId == employeeId)))).OrderByDescending(s => s.EndTime).FirstOrDefaultAsync();
    ...
    private static CoverageStatus DetermineStatus(CoverageResult result, bool understaffed) => result.Tasks.Any(x => x.Critical && x.Gaps.Any()) ? CoverageStatus.Red : understaffed || result.Tasks.Any(x => x.Gaps.Any()) ? CoverageStatus.Yellow : CoverageStatus.Green;
}
