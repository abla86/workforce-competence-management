...
public sealed class MigrationService
{
    ...
    public async Task<MigrationImportResult> ImportAsync(MigrationImportRequest request, string actor, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var competences = await _db.Competences.ToListAsync(cancellationToken);
            var employees = await _db.Employees.Include(x => x.Competences).ToListAsync(cancellationToken);
            var shifts = await _db.Shifts.Include(x => x.Assignments).Include(x => x.Requirements).ToListAsync(cancellationToken);

            var competenceMap = competences.ToDictionary(x => Normalize(x.Name));
            var employeeMap = employees.ToDictionary(x => EmployeeKey(x.Name, x.Role));
            var shiftMap = shifts.ToDictionary(x => ShiftKey(x.Date, x.StartTime, x.ShiftType, x.Department));
            ...
            foreach (var source in request.Competences)
            {
                ...
                if (competenceMap.TryGetValue(key, out var existing))
                {
                    ...
                    existing.Category = source.Category?.Trim() ?? existing.Category;
                    updated++;
                }
                else
                {
                    var item = new Competence { Name = source.Name.Trim(), Category = string.IsNullOrWhiteSpace(source.Category) ? "General" : source.Category.Trim() };
                    _db.Competences.Add(item); competenceMap[key] = item; created++;
                }
            }

            await _db.SaveChangesAsync(cancellationToken);
            competences = await _db.Competences.ToListAsync(cancellationToken);
            competenceMap = competences.ToDictionary(x => Normalize(x.Name));

            foreach (var source in request.Employees)
            {
                var key = EmployeeKey(source.Name, source.Role);
                if (string.IsNullOrWhiteSpace(Normalize(source.Name)) || string.IsNullOrWhiteSpace(Normalize(source.Role)))
                { conflicts.Add(new("Employee", source.Name, "Navn og rolle er påkrevd")); continue; }
                ...
                if (employeeMap.TryGetValue(key, out var existing))
                {
                    ...
                    existing.Department = source.Department?.Trim() ?? existing.Department;
                    existing.Authorization = source.Authorization?.Trim();
                    existing.PositionPercent = source.PositionPercent is > 0 and <= 100 ? source.PositionPercent.Value : existing.PositionPercent;
                    existing.MaxWeeklyHours = source.MaxWeeklyHours is > 0 and <= 80 ? source.MaxWeeklyHours.Value : existing.MaxWeeklyHours;
                ...
                else
                {
                    ...
                    {
                        Name = source.Name.Trim(), Role = source.Role.Trim(), Department = source.Department?.Trim() ?? "",
                        Authorization = source.Authorization?.Trim(), PositionPercent = source.PositionPercent is > 0 and <= 100 ? source.PositionPercent.Value : 100m,
                        MaxWeeklyHours = source.MaxWeeklyHours is > 0 and <= 80 ? source.MaxWeeklyHours.Value : 37.5m,
            ...
            await _db.SaveChangesAsync(cancellationToken);
            employees = await _db.Employees.Include(x => x.Competences).ToListAsync(cancellationToken);
            employeeMap = employees.ToDictionary(x => EmployeeKey(x.Name, x.Role));
            ...
            foreach (var source in request.Employees)
            {
                ...
                {
                    if (!competenceMap.TryGetValue(Normalize(ec.Name), out var competence))
                    { conflicts.Add(new("EmployeeCompetence", $"{source.Name}/{ec.Name}", "Kompetansen finnes ikke")); continue; }
                    var existing = await _db.EmployeeCompetences.FindAsync([employee.Id, competence.Id], cancellationToken);
                    if (existing is null)
                        _db.EmployeeCompetences.Add(new EmployeeCompetence { EmployeeId = employee.Id, CompetenceId = competence.Id, Level = ec.Level, ValidUntil = ec.ValidUntil });
                    else if (request.Mode == MigrationConflictMode.Update)
                    { existing.Level = ec.Level; existing.ValidUntil = ec.ValidUntil; }
                }
            }
            ...
            foreach (var source in request.Shifts)
            {
                ...
                if (shiftMap.TryGetValue(key, out var existing))
                {
                    ...
                    existing.Hours = source.Hours; existing.MinimumStaff = source.MinimumStaff; existing.IsCritical = source.IsCritical; existing.IsPublished = source.IsPublished;
                    updated++;
            ...
            await Audit(db: _db, action: "migration.import.completed", entityType: "Migration", entityId: Guid.NewGuid().ToString("N"), reason: request.SourceFileName, actor, details);
            await _db.SaveChangesAsync(cancellationToken);
    ...
    {
        foreach (var assignment in source.Assignments.Distinct(StringComparer.OrdinalIgnoreCase))
            if (employees.TryGetValue(NormalizeEmployeeKey(assignment), out var employee) && !shift.Assignments.Any(x => x.EmployeeId == employee.Id))
                _db.ShiftAssignments.Add(new ShiftAssignment { ShiftId = shift.Id, EmployeeId = employee.Id });
        ...
        foreach (var requirement in source.Requirements)
        {
            ...
            if (existing is null)
                _db.ShiftRequirements.Add(new ShiftRequirement { ShiftId = shift.Id, CompetenceId = competence.Id, MinimumCount = requirement.MinimumCount, MinimumLevel = requirement.MinimumLevel, RequiredRole = requirement.RequiredRole, IsCritical = requirement.IsCritical });
            else
            { existing.MinimumCount = requirement.MinimumCount; existing.MinimumLevel = requirement.MinimumLevel; existing.RequiredRole = requirement.RequiredRole; existing.IsCritical = requirement.IsCritical; }
        }
    }

    private static string NormalizeEmployeeKey(string value) => value.Contains('|') ? value : value;
    private static string EmployeeKey(string name, string role) => $"{Normalize(name)}|{Normalize(role)}";
    private static string ShiftKey(DateOnly date, TimeOnly? start, string type, string? department) => $"{date:yyyy-MM-dd}|{start:HH\\:mm}|{Normalize(type)}|{Normalize(department ?? "")}";
    private static string Normalize(string? value) => (value ?? "").Trim().ToLowerInvariant();

    private static async Task Audit(AppDbContext db, string action, string entityType, string entityId, string? reason, string actor, string? details)
    {
        db.AuditEvents.Add(new AuditEvent { OccurredAtUtc = DateTime.UtcNow, Action = action, EntityType = entityType, EntityId = entityId, Reason = reason, Actor = actor, DetailsJson = details });
        await Task.CompletedTask;
    }

    public static async Task<IReadOnlyList<IReadOnlyList<string>>> ReadExcelAsync(Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var sheet in workbook.Workbook.Sheets!.Elements<Sheet>())
        {
            ...
            {
                var values = row.Elements<Cell>().Select(cell => GetCellValue(cell, workbook, sharedStrings)).ToList();
    ...
    private static string GetCellValue(Cell cell, WorkbookPart workbook, SharedStringTable? sharedStrings)
    {
...
{
    public List<MigrationEmployeeCompetence> Competences { get; init; } = Competences ?? [];
}
...
{
    public List<string> Assignments { get; init; } = Assignments ?? [];
    public List<MigrationRequirement> Requirements { get; init; } = Requirements ?? [];
}
