...
public static class WorkforceExpansionEndpoints
{
    ...
    {
        app.MapPut("/api/employees/{id:int}/availability", async (
            int id,
            SetEmployeeAvailabilityRequest request,
            AppDbContext db) =>
        {
        ...
        app.MapGet("/api/employees/{id:int}/availability", async (
            int id,
            AppDbContext db) =>
        {
            var exists = await db.Employees.AnyAsync(x => x.Id == id);
            if (!exists)
                return Results.NotFound();
        ...
        app.MapDelete("/api/employees/{id:int}/availability/{date}", async (
            int id,
            DateOnly date,
            AppDbContext db) =>
        {
        ...
        app.MapGet("/api/shifts/{id:int}/candidates", async (
            int id,
            AppDbContext db) =>
        {
