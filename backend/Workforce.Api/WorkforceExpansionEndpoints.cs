...
public static class WorkforceExpansionEndpoints
{
    public static void MapWorkforceExpansionEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/employees/{id:int}/status", async (int id, DateTime? date, AppDbContext db, EmployeeAccessService access, HttpContext http) =>
        {
            if (!await access.CanAccessEmployeeAsync(http.User, id)) return Results.Forbid();
            var service = new EmployeeAvailabilityService(db, new NotificationService(db));
            return Results.Ok(await service.GetEmployeeStatusAsync(id, date ?? DateTime.Today));
        }).RequireAuthorization("CoverageRead");
        ...
        {
            var service = new ShiftPlanService(db, new NotificationService(db));
            var employees = await service.GetWhoIsWorkingTodayAsync(departmentId, date ?? DateTime.Today);
            return Results.Ok(employees.Select(e => new { id = e.Id, name = e.Name, role = e.Role, shifts = e.ShiftAssignments.Where(a => a.Shift.StartTime.Date == (date ?? DateTime.Today).Date).Select(a => new { a.ShiftId, a.Shift.StartTime, a.Shift.EndTime, a.Shift.ShiftType }) }));
        }).RequireAuthorization("CoverageRead");
        ...
        app.MapGet("/api/dailyplans/today/{departmentId:int}", async (int departmentId, AppDbContext db) =>
        {
            ...
            var plan = await service.GetTodayPlanAsync(departmentId) ?? await service.CreateDailyPlanAsync(departmentId, "system");
        ...
        {
            var service = new DailyPlanService(db, new NotificationService(db));
            var plan = await service.GetTodayPlanAsync(departmentId) ?? await service.CreateDailyPlanAsync(departmentId, user.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown");
            await service.PublishDailyPlanAsync(plan.Id, user.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown");
        ...
        {
            var service = new DailyPlanService(db, new NotificationService(db));
            await service.AddTaskToDailyPlanAsync(planId, new DailyTaskItem { Title = request.Title.Trim(), Description = request.Description, StartTime = request.StartTime, EndTime = request.EndTime, SortOrder = request.SortOrder }, user.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown");
        ...
        app.MapPost("/api/shiftplans", async (CreateShiftPlanRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            var service = new ShiftPlanService(db, new NotificationService(db));
            return Results.Ok(await service.CreateShiftPlanAsync(request.DepartmentId, request.StartDate, request.EndDate, user.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown"));
        }).RequireAuthorization("CoverageManage");
        ...
        {
            if (id != request.ShiftId) return Results.BadRequest(new { message = "Path shift id and request shift id must match." });
            return Results.Ok(await new AutoStaffingService(db).GenerateAsync(request));
        }).RequireAuthorization("CoverageManage");
        ...
        app.MapGet("/api/shifts/{id:int}/candidates", async (int id, AppDbContext db) =>
        {
            ...
            return Results.Ok(await new ShiftMatchingService(db).FindCandidatesAsync(id));
        }).RequireAuthorization("CoverageManage");
        ...
        app.MapGet("/api/notifications", async (ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            var employee = await db.Employees.FirstOrDefaultAsync(e => e.IdentitySubject == subject);
            if (employee is null) return Results.Ok(Array.Empty<object>());
        ...
        app.MapPut("/api/notifications/{id:int}/read", async (int id, ClaimsPrincipal user, AppDbContext db) =>
        {
            var subject = user.FindFirstValue(ClaimTypes.NameIdentifier) ?? user.FindFirstValue("sub");
            var employee = await db.Employees.FirstOrDefaultAsync(e => e.IdentitySubject == subject);
            var item = employee is null ? null : await db.Notifications.FirstOrDefaultAsync(n => n.Id == id && n.EmployeeId == employee.Id);
            if (item is null) return Results.NotFound();
            item.IsRead = true; await db.SaveChangesAsync(); return Results.NoContent();
        }).RequireAuthorization("CoverageRead");

        app.MapGet("/api/absences/{employeeId:int}", async (int employeeId, AppDbContext db, EmployeeAccessService access, HttpContext http) =>
        {
            if (!await access.CanAccessEmployeeAsync(http.User, employeeId)) return Results.Forbid();
            return Results.Ok(await db.Absences.Where(a => a.EmployeeId == employeeId).OrderByDescending(a => a.StartDate).ToListAsync());
        }).RequireAuthorization("CoverageRead");

        app.MapPost("/api/absences", async (AbsenceRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            var item = new Absence { EmployeeId = request.EmployeeId, Type = request.Type, StartDate = request.StartDate, EndDate = request.EndDate, Description = request.Description, IsApproved = false };
            if (item.EndDate <= item.StartDate) return Results.BadRequest(new { message = "EndDate must be after StartDate." });
            db.Absences.Add(item); await db.SaveChangesAsync(); return Results.Created($"/api/absences/{item.Id}", item);
        }).RequireAuthorization("CoverageManage");

        app.MapPut("/api/absences/{id:int}/approve", async (int id, ClaimsPrincipal user, AppDbContext db) =>
        {
            var item = await db.Absences.FindAsync(id); if (item is null) return Results.NotFound();
            item.IsApproved = true; item.ApprovedBySubject = user.FindFirstValue(ClaimTypes.NameIdentifier); item.ApprovedAt = DateTime.UtcNow;
        ...
        app.MapGet("/api/dispensations", async (int? employeeId, AppDbContext db) =>
        {
            var query = db.ShiftDispensations.AsQueryable(); if (employeeId.HasValue) query = query.Where(x => x.EmployeeId == employeeId.Value);
            return Results.Ok(await query.OrderByDescending(x => x.GrantedAt).Take(100).ToListAsync());
        }).RequireAuthorization("CoverageManage");

        app.MapPost("/api/dispensations", async (DispensationRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            if (request.HoursGranted <= 0) return Results.BadRequest(new { message = "HoursGranted must be greater than zero." });
            var item = new ShiftDispensation { EmployeeId = request.EmployeeId, ShiftId = request.ShiftId, BreachedRule = request.BreachedRule, HoursGranted = request.HoursGranted, Reason = request.Reason.Trim(), GrantedBySubject = user.FindFirstValue(ClaimTypes.NameIdentifier) ?? "unknown", Status = DispensationStatus.Pending, ExpiresAt = request.ExpiresAt };
            db.ShiftDispensations.Add(item); await db.SaveChangesAsync(); return Results.Created($"/api/dispensations/{item.Id}", item);
        }).RequireAuthorization("CoverageManage");

        app.MapPut("/api/dispensations/{id:int}/decision", async (int id, DispensationDecisionRequest request, AppDbContext db) =>
        {
            var item = await db.ShiftDispensations.FindAsync(id); if (item is null) return Results.NotFound();
            item.Status = request.Approve ? DispensationStatus.Approved : DispensationStatus.Rejected; item.Comments = request.Comments; await db.SaveChangesAsync(); return Results.Ok(item);
        }).RequireAuthorization("CoverageManage");
    }
}
