...
app.MapGet("/api/me/data-export", async (HttpContext http, AppDbContext db) =>
{
    ...
    var employee = await db.Employees
        .Include(e => e.Competences).ThenInclude(ec => ec.Competence)
        .Include(e => e.Availability)
        .Include(e => e.ShiftAssignments).ThenInclude(a => a.Shift)
        .SingleOrDefaultAsync(e => e.IdentitySubject == subject);
...
app.MapGet("/api/me/privacy-requests", async (HttpContext http, AppDbContext db) =>
{
    var subject = http.User.FindFirstValue(ClaimTypes.NameIdentifier);
    if (string.IsNullOrWhiteSpace(subject)) return Results.Unauthorized();
    return Results.Ok(await db.PrivacyRequests
        .Where(x => x.IdentitySubject == subject)
        .OrderByDescending(x => x.RequestedAt)
        .Select(x => new { x.Id, x.Type, x.RequestedAt, x.Status, x.CompletedAt })
        .ToListAsync());
})
.RequireAuthorization("CoverageRead")
.WithTags("privacy");
