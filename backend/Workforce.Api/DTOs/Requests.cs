...
public sealed record CreateEmployeeRequest(string Name, string Role, decimal PositionPercent, decimal? MaxWeeklyHours = null);
public sealed record UpdateEmployeeRequest(string Name, string Role, decimal PositionPercent, bool IsActive = true, decimal? MaxWeeklyHours = null);
public sealed record AddCompetenceRequest(int CompetenceId, CompetenceLevel Level, DateOnly? ValidUntil);
...
public sealed record AssignEmployeeRequest(int EmployeeId, string? OverrideReason = null);
public sealed record AddRequirementRequest(int CompetenceId, int MinimumCount, CompetenceLevel MinimumLevel, string? RequiredRole = null, bool IsCritical = false);
